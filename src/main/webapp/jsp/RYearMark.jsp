...
//                $('#subjectno').append("<option value='noselected'>请选择学科</option>");
    ...
    function initMarkChart(data) {//各科成绩图表
        ...
        for (var i = 0; i < data.length; i++) {
            classData.push(data[i].classno);

            var item = {
                name: data[i].classno,
                type: 'line',
                data: [data[i].markone, data[i].marktwo, data[i].markthree, data[i].markfour, data[i].markfive, data[i].marksix, data[i].markseven, data[i].markeight, data[i].marknine, data[i].markten, data[i].markeleven, data[i].marktwevle, data[i].markmidterm, data[i].markfinal, data[i].avemark]
            }
            serie.push(item);
        }
