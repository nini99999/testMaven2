...
var XDate = function (g, q, E, t) {
    function h() {
        var a = this instanceof h ? this : new h, c = arguments, b = c.length;
        if ("boolean" == typeof c[b - 1]) {
            var d = c[--b];
            c = u(c, 0, b)
        }
        if (b)if (1 == b)if (b = c[0], b instanceof g) a[0] = new g(b.getTime()); else if ("number" == typeof b) a[0] = new g(b); else if (b instanceof h) {
            var c = a, f = new g(b[0].getTime());
            n(b) && (f.toString = z);
            c[0] = f
        } else {
            if ("string" == typeof b) {
                a[0] = new g(0);
                a:{
                    for (var c = b, b = d || !1, f = h.parsers, A = 0, e; A < f.length; A++)if (e = f[A](c, b, a)) {
                        a = e;
                        break a
                    }
                    a[0] = new g(c)
    ...
    function O(a, c, b, d, f) {
        var e = h.locales, g = e[h.defaultLocale] || {}, P = l(k, a, f);
        b = ("string" == typeof b ? e[b] : b) || {};
        return B(a, c, function (a) {
            if (d)for (var b = (7 == a ? 2 : a) - 1; 0 <= b; b--)d.push(P(b));
            return P(a)
        }, function (a) {
            return b[a] || g[a]
        }, f)
    ...
    var r = "FullYear Month Date Hours Minutes Seconds Milliseconds Day Year".split(" "), R = ["Years",
        "Months", "Days"], K = [12, 31, 24, 60, 60, 1E3, 1], S = /(([a-zA-Z])\2*)|(\((('.*?'|\(.*?\)|.)*?)\))|('(.*?)')/, p = g.UTC, z = g.prototype.toUTCString, e = h.prototype;
    e.length = 1;
    ...
    h.formatters = {i: "yyyy-MM-dd'T'HH:mm:ss(.fff)", u: "yyyy-MM-dd'T'HH:mm:ss(.fff)zzz"};
